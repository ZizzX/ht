...
window.addEventListener("DOMContentLoaded",()=>{const e=document.querySelector("#form"),t=e.querySelector("#nights"),n=e.querySelector("#nights-to"),r=e.querySelector("#depart-city"),o=e.querySelector("#country"),i=document.querySelector("#tourTableBody"),a=document.querySelector(".tour-table thead"),c=document.querySelector(".tours__title");function d(e,t){return e.price>t.price?1:e.price<t.price?-1:0}function l(e,t){e.value="",e.setAttribute("placeholder",t),e.classList.add("error")}function u(e){e.classList.remove("error"),e.setAttribute("placeholder","от 5 до 15")}t.addEventListener("input",e=>{const t=e.target;n.value.length>0&&+n.value<=+t.value?l(t,"Больше чем до"):n.value>t.value&&u(t)}),n.addEventListener("change",e=>{const n=e.target;t.value.length>0&&+t.value>=+n.value?l(n,"Меньше чем от"):n.value>t.value&&u(n)}),e.addEventListener("submit",e=>{e.preventDefault();const l=Object.fromEntries(new FormData(e.target).entries()),{departCity:s,country:h,date:p,nights:v,nightsTo:y}=l;(async({departCity:e,country:t,date:n,nights:r,nightsTo:o}={})=>{const i=await fetch(`https://ht.kz/test/searchPartner1?departCity=${e}&country=${t}&date=${n}&nights=${r}&nightsTo=${o}`);return await i.json()})({departCity:s,country:h,date:p,nights:v,nightsTo:y}).then(e=>{const t=e.tours.sort(d);let n=r[r.options.selectedIndex].innerText,l=o[o.options.selectedIndex].innerText,u=new Date(p).toLocaleDateString("lookup",{year:"numeric",month:"long",day:"numeric"}).replace(/г.$/g,"").trim();c.innerHTML=`\n      <h2>\n        Результаты запроса (${n} -\n        ${l},\n        ${u},\n        ${v}-${y}н.):\n      </h2>\n      `,a.innerHTML="\n        <tr>\n          <th>Hotel name</th>\n          <th>price</th>\n          <th>currency</th>\n        </tr>\n      ",t.forEach(e=>{i.innerHTML+=`\n        <tr>\n          <td>${e.hotelName}</td>\n          <td>${e.price}</td>\n          <td>${e.currency}</td>\n        </tr>\n        `})}).catch(e=>{throw new Error(`${e.name} ${e.message}`)}).finally(()=>{e.target.reset(),u(t),u(n)})}),r.addEventListener("change",e=>{const t=e.target;t&&"10"===r.value?o.innerHTML='\n      <option value="" disabled selected hidden>Страна</option>\n      <option value="552">Турция</option>\n      <option value="553">Тайланд</option>':t&&"11"===r.value?o.innerHTML='\n      <option value="" disabled selected hidden>Страна</option>\n      <option value="552">Турция</option>\n      <option value="554">Чехия</option>':o.innerHTML='\n      <option value="" disabled selected hidden>Страна</option>\n      <option>Страна</option>\n      '})});
